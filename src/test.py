...
n_box = 3
color = 'green'
mask = LabelFrame(root, width=(2*n_box+1)*MAP.BOX_DIM, height=(2*n_box+1)*MAP.BOX_DIM)

y = (n_box - n_box) * MAP.BOX_DIM

n_box_row = 0       # number of boxes to create on the current row
for i in range(2*n_box + 1):
    x = (n_box - n_box_row) * MAP.BOX_DIM
    ...
    for j in range(2*n_box_row + 1):
        ...
        if not skip:
            rect = Label(mask, bg=color, padx=MAP.BOX_DIM//2, height=0, relief='solid')
...
# mask.place(x=(box_x - n_box) * MAP.BOX_DIM, y=(box_y - n_box_row) * MAP.BOX_DIM, anchor=NW)
