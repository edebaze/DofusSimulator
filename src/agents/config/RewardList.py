...
class RewardList:
    ...
    REACH_MAX_TURN = -1000      # reward for ending game with max turn

    DAMAGES = 3                 # reward for doing 1 damage
    HP_LOSS = -1                # reward for losing 1 hp

    ROUND_START = -10            # reward when the round starts
    BAD_MOVEMENT = -10          # reward for making a bad movement action
    BAD_SPELL_CASTING = -30     # reward for casting a spell without being in enemy's range
    BAD_SPELL_SELECTION = -10   # reward for trying to cast a spell without the necessary amount of PA
